...
export default function RegisterAgencyPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // 2. Insert user dulu (agency_id null sementara)
      ...
      // 4. Update user dengan agency_id
      const { error: updateError } = await supabase.from("users").update({ agency_id: agencyData.id }).eq("id", authData.user.id);

      if (updateError) {
        setError("Gagal menghubungkan akun dengan agensi. Silakan coba lagi.");
        return;
      }
